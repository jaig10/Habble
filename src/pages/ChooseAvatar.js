    ...
    <>
      <div className='m-6'>
        ...
        <div className='flex flex-col md:flex-row gap-4 items-center justify-center'>
          { avatars.map(avatar=>{
            return(
              <div key={avatar.name} className={`bg-peach flex flex-col items-center gap-2 h-[242px] w-[242px] rounded-3xl md:h-[279px] md:w-[300px] hover:border-2 hover:border-bluish ${selectedAvatar===avatar.name? "border-2 border-bluish":""}`} onClick={()=>{setSelectedAvatar(avatar.name)}}>
            <div>
        ...
        <button className="css-button-sharp--blue" onClick={()=>{navigate("/talk", {state:{botPersonality: selectedAvatar}})}}>NEXT</button>
      </div>
      </div>
    </>
  )
